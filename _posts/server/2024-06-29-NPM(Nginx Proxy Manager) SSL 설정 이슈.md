...
1️: 도커로 올라간 컨테이너에 접속한다.

`docker exec -it <컨테이너 명> /bin/bash`
  
2️: `certbot`을 입력해서 Cloudflare-DNS 플러그인에서 이슈가 발생하는지 확인한다.
...
3️: `pip uninstall certbot-dns-cloudflare` 명령을 입력하여 `certbot-dns-cloudflare` 패키지를 삭제한다.
  
4️: 다시 한번 `certbot`을 입력하여 오류가 출력되는지 확인한다.
...
5️: 이번엔 방금 삭제했던 `certbot-dns-cloudflare`를 설치합니다.
